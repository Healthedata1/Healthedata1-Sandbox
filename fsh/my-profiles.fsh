...
* characteristic.value[x] ^comment = "do we need to constrain this list - can everybody support all the types if they support the characteristic element or for example do we need to you only need to must support just code and boolean"
...
* member MS
* member.entity only Reference($USPatient)
* member.entity MS
...
* #location         "Location"         "Patients are all at the location specified by the value which names the physical structures managed/operated by an organization (e.g., a ward, building, clinic, etc)"
